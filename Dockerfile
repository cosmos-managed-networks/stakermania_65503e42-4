...
# then log output of file /code/bin/stmd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/stmd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/stmd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/stmd /usr/bin/stmd
