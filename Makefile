...
HTTPS_GIT := github.com/stakemania/stakermania.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=stakermania \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=stmd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/stmd ./cmd/stmd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/stmd.exe ./cmd/stmd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/stmd
...
draw-deps:
	...
	@goviz -i ./cmd/stmd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c stakermania --local -f chains.yaml
endif
...
set-testnet-configs:
	stmd config set client chain-id chainid-1
	stmd config set client keyring-backend test
	stmd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | stmd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | stmd keys add acc1 --recover`
